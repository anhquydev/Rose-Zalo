...
ALLOWED_GROUPS_FILE = "modules/cache/sendtask_autosticker.json"
...
def handle_autosend_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(command_parts) != 2:
         response_message = "Sử dụng: .autosticker on/off"
    else:
        ...
        if action == "on":
            if thread_id not in allowed_groups:
                ...
                response_message = f"Đã bật AutoSend cho nhóm này!"
                  ...
                  response_message = f"Nhóm này đã được bật AutoSticker trước đó!"
        elif action == "off":
            if thread_id in allowed_groups:
                allowed_groups.remove(thread_id)
                allowed_groups_data["groups"] = allowed_groups
                save_allowed_groups(allowed_groups_data)
                response_message = f"Đã tắt AutoSticker cho nhóm này"
            else:
                response_message = f"Nhóm này đã được tắt AutoSticker trước đó rồi!"
        else:
            response_message ="Sử dụng: autosticker on/off"
...
def ft_vxkiue():
    return {
         'autosticker': handle_autosend_command
    }