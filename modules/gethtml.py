...
des = {
    ...
    'description': "Lấy HTML của trang web và gửi về",
...
def handle_gethtml_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(parts) != 2:
        send_message(client, thread_id, thread_type, "Lệnh không đúng định dạng. Vui lòng sử dụng: `gethtml <url>`")
        return
...
def ft_vxkiue():
    return {
        'gethtml': handle_gethtml_command
    }